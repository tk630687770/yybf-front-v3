    ...
    <section v-if="prepare" class="card">
      ...
      <div v-if="!historyCollapsed && selectedHistoryWindow === 'ALL'" class="history-window-grid">
        <article v-for="window in prepare.windows" :key="window.windowCode" class="history-card">
          ...
          <template v-if="historyViewMode[window.windowCode] === 'year'">
            <div class="state-tabs"><button v-for="item in historyItems(window.windowCode)" :key="item.state" class="history-tab" :class="{ active: activeHistoryState[window.windowCode] === item.state }" @click="activeHistoryState[window.windowCode] = item.state"><span v-for="(part, index) in stateParts(item.state)" :key="index" :class="{ 'hit-count-badge': part.badge }">{{ part.text }}</span>：{{ item.count }}<small>今{{ item.currentYearCount }}</small></button></div>
      ...
      <div v-if="!historyCollapsed && selectedHistoryWindow !== 'ALL'" class="history-state-grid">
        <article v-for="item in selectedHistoryItems" :key="item.state" class="history-card">
          <div class="history-card-head"><strong><span v-for="(part, index) in stateParts(item.state)" :key="index" :class="{ 'hit-count-badge': part.badge }">{{ part.text }}</span></strong><span class="history-meta">全{{ item.count }}/今{{ item.currentYearCount }}</span></div>
...
function stateParts(state: string) {
  return state.split(/(\*\d+)/).filter(Boolean)
    .flatMap(part => part.startsWith('*') ? [{ text: '*', badge: false }, { text: part.slice(1), badge: true }] : [{ text: part, badge: false }]);
}
...
.hit-count-badge{display:inline-block;min-width:14px;margin:0 1px;padding:0 3px;border-radius:999px;background:rgba(245,196,76,.24);color:#f5c451;font-size:10px;font-weight:700;line-height:14px;text-align:center;vertical-align:super}
</style>
