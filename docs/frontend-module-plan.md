...
5. 历史回放实验台：承接开奖前视角的批量历史回放，用保存实验替代靠等待未来期数缓慢验证。
6. 红球入口召回研究台：承接第一层入口海选的组件化召回、逐球证据、组合实验和稳定性分析。
...
- `/historical-replay-lab`：历史回放实验台。
- `/entry-recall-lab`：红球入口召回研究台。
...
- 历史回放运行参数。
- `OFFICIAL_V1` 等策略版本的批量回放。
- 保存历史实验结果。
- 已保存实验列表。
- 实验详情读取。
- 总收益、红球命中、蓝球命中、奖级分布和逐期明细展示。

红球入口召回研究台承载内容：

- 独立入口组件、权重、标准化、组合算法、TopN和历史范围参数。
- 临时预览、正式保存、重复指纹提示和正式实验库。
- Top15/18/20等入口规模统一召回指标。
- 逐期入口池、遗漏号码、边界分和33球逐组件证据。
- 多实验横向对比、连续时间切片稳定性和两两权重网格。
- 只研究入口召回，不承担9红压缩、10注出票和收益评价。
...
目标是建立一个相对独立的历史开奖前视角回放入口，让策略验证不再只能依赖未来开奖慢慢积累样本。

已完成动作：

- 新增 `src/api/modules/historicalReplay.ts`。
- 新增 `/historical-replay-lab` 历史回放实验台。
- 接入 `/ssq/window/axis/replay/historical/run`。
- 接入 `/ssq/window/axis/replay/historical/list`。
- 接入 `/ssq/window/axis/replay/historical/detail`。
- 展示实验摘要、收益指标、红球命中分布、奖级分布、逐期明细和已保存实验列表。
- 新增实验横向对比区，支持把两个已保存实验放在同一视图比较核心指标和入口融合提示。

后续待补：

- 增加策略参数差异展示。
- 增加更细的逐期差异筛选，例如只看B优于A、A优于B或入口救回但票面丢失的期。
- 增加逐期明细分页或筛选。
- 将策略模块化后，允许页面选择不同策略组件组合并生成实验。

### 第七阶段：红球入口召回研究台

目标是先证明第一层入口海选能力，再恢复后续压缩和出票研究。

已完成动作：

- 新增 `src/api/modules/entryRecall.ts`，封装运行、列表、详情、对比、稳定性和网格预览接口。
- 新增 `/entry-recall-lab` 独立页面，不挤入实时预测、快照复盘或普通诊断研究台。
- 参数、指标、逐期逐球证据、实验库与对比、稳定性、权重网格拆为独立组件。
- 临时预览不落库，正式保存读取后端专用入口召回实验表。
- 新增 `EntryParallelPredictionModal.vue`，在入口召回实验库上方提供“查看预测”入口，集中展示入口实验拟正式预测快照。
- 第一版拟正式预测弹窗只承载入口池观察线，压缩池、9+1、10注6+1和蓝球后续由严格前视下一期运行器补齐。

后续研究动作：

- 在统一100期范围保存全部独立组件基线和代表性组合实验。
- 重点比较0至2红、至少5红、完整6红、随机基线提升和跨时间切片稳定性。
- 根据逐球证据识别持续遗漏类型，达到现有组件瓶颈后再开发新入口组件。
- 补后端严格开奖前视角入口实验下一期运行器，让入口拟正式预测能面向未来期生成入口池，而不是只能读取历史实验逐期证据。
