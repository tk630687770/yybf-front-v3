...
2. 模型预测/诊断结果台：承接后端评分、候选池、回测、诊断和最终票面输出。
...
模型预测/诊断结果台是新增主线，核心目标是“验证模型是否有价值，并把预测结果可解释地展示出来”。

建议路由：

- `/model-dashboard`
- `/model-dashboard/predict`
- `/model-dashboard/backtest`
- `/model-dashboard/diagnosis`
