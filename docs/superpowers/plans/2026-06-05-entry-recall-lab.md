...
| Task 5：权重网格与页面编排 | 已完成 |
| Task 6：路由、文档、构建和浏览器验证 | 已完成 |
...
- `npm run build` 通过，Vue类型检查与Vite生产构建成功。
- 后端正式实验列表接口实际返回实验ID 2。
- 后端稳定性接口实际返回100期、4个连续时间切片。
- 后端网格接口使用3个组件、2种算法、3组权重、20期数据实际产生18个候选，耗时约2.4秒。
- Codex内置浏览器验证 `/entry-recall-lab` 路由、顶部导航、页面身份、正式实验详情、统一指标和逐期逐球证据均正常。
- 浏览器继续点击稳定性和网格时受到内置浏览器本地地址安全策略限制；没有绕过限制，改用真实后端接口完成剩余验证。
